...
$sections = get_post_meta( $post->ID, '__section', true);
$background = array();
if ( isset($sections) && is_array($sections) && count($sections) > 0 ) {

	foreach ( $sections as $section ) {

		if ( isset($section['background']) && is_array($section['background']) && count($section['background']) > 0 ) {
			
			foreach ( $section['background'] as $k => $val ) {
