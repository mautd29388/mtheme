...
function mTheme_disable_comments_page() {
	remove_post_type_support( 'page', 'comments' );
}
add_action( 'init', 'mTheme_disable_comments_page');


/**
 * Get link post_type and page number
 * */
function mTheme_get_post_type_archive_pagenum_link( $post_type, $max_page, $pagenum, $escape = true ) {
	global $wp_rewrite;
...
function mTheme_comment_form_fields( $fields ) {
	return array(
			'author' => '<p class="comment-form-author">' . '<label for="author">' . __( 'Name', 'mTheme' ) . '</label> ' .
			'<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" required="required" /></p>',
			'email'  => '<p class="comment-form-email"><label for="email">' . __( 'Email', 'mTheme' ) . '</label> ' .
			'<input id="email" name="email" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" aria-describedby="email-notes" required="required" /></p>',
	);
}
		...
		<<?php echo $tag ?> <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ) ?> id="comment-<?php comment_ID() ?>">
			<article class="comment-body">
				...
				<div class="comment-meta">
					<p>by <?php echo get_comment_author_link(); ?> on <span class="comment-date"><?php echo get_comment_date(); ?></span></p>
				</div>
